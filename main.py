...
async def main(page: ft.Page):
    ...
    async def persist_session():
        await page.client_storage.set_async(TOKEN_KEY, api_client.access_token or "")
        await page.client_storage.set_async(REFRESH_KEY, api_client.refresh_token or "")
        if api_client.user:
            await page.client_storage.set_async(ROLE_KEY, api_client.user.get("role", ""))
            await page.client_storage.set_async(EMAIL_KEY, api_client.user.get("email", ""))

    async def clear_session():
        for k in [TOKEN_KEY, REFRESH_KEY, ROLE_KEY, EMAIL_KEY]:
            await page.client_storage.remove_async(k)

    async def restore_session():
        token = await page.client_storage.get_async(TOKEN_KEY)
        if not token:
            return False
        api_client.access_token = token
        api_client.refresh_token = await page.client_storage.get_async(REFRESH_KEY)
        role = await page.client_storage.get_async(ROLE_KEY)
        email = await page.client_storage.get_async(EMAIL_KEY)
