...
def client_paiements_page(page: ft.Page, on_navigate, on_logout):
    ...
    def open_new_paiement_dialog():
        ...
        reservation_dropdown = ft.Dropdown(
            label="Réservation", width=320, bgcolor=COLOR_BG, color=COLOR_TEXT, border_color=COLOR_BORDER,
            options=[ft.dropdown.Option(key=str(r["id"]), text=f"#{r['id']} - {r.get('nom_defunt', '')}") for r in reservations_validees],
        )
        montant_field = ft.TextField(label="Montant (FCFA)", width=320, bgcolor=COLOR_BG, color=COLOR_TEXT, border_color=COLOR_BORDER, keyboard_type=ft.KeyboardType.NUMBER)
        canal_dropdown = ft.Dropdown(
            label="Canal de paiement", width=320, bgcolor=COLOR_BG, color=COLOR_TEXT, border_color=COLOR_BORDER,
            options=[
        ...
        def handle_submit(e):
            if not reservation_dropdown.value or not montant_field.value or not canal_dropdown.value:
                ...
                return
            try:
                payload = {"reservation_id": int(reservation_dropdown.value), "montant": float(montant_field.value), "canal": canal_dropdown.value}
        ...
        page.dialog = ft.AlertDialog(
            ...
            content=ft.Column([reservation_dropdown, montant_field, canal_dropdown, error_text, success_text], spacing=12, tight=True, width=340, scroll=ft.ScrollMode.AUTO),
            actions=[
    ...
    def build_row(p):
        ...
        return ft.Container(
            content=ft.Row(
                [
                    ft.Column([ft.Text(p.get("reference", "-"), size=14, weight=ft.FontWeight.W_600, color=COLOR_TEXT), ft.Text(f"{montant:,.0f} FCFA • {canal}", size=12, color=COLOR_TEXT_MUTED)], spacing=2, expand=True),
    ...
    def refresh_list():
        ...
        if not paiements or not isinstance(paiements, list):
            list_container.controls.append(ft.Text("Vous n'avez effectué aucun paiement", color=COLOR_TEXT_MUTED, size=14))
        else:
            for p in paiements:
    ...
    header_controls.append(ft.ElevatedButton("Nouveau paiement", icon=ft.Icons.ADD, bgcolor=COLOR_PRIMARY, color=COLOR_TEXT, on_click=lambda e: open_new_paiement_dialog()))
