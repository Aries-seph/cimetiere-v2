...
def client_reserver_page(page: ft.Page, on_navigate, on_logout, preselect_caveau_id=None):
    ...
    def open_reservation_dialog(caveau):
        ...
        date_deces_field = ft.TextField(label="Date de décès", width=260, bgcolor=COLOR_BG, color=COLOR_TEXT, border_color=COLOR_BORDER, read_only=True)
        ...
        date_picker = ft.DatePicker(first_date=datetime(1900, 1, 1), last_date=datetime.now(), on_change=handle_date_change)
        ...
        commentaire_field = ft.TextField(label="Commentaire (optionnel)", width=320, multiline=True, min_lines=2, bgcolor=COLOR_BG, color=COLOR_TEXT, border_color=COLOR_BORDER)
        ...
        return ft.Container(
            content=ft.Row(
                [
                    ft.Column(
                        [
                            ft.Row([ft.Container(width=10, height=10, bgcolor=COLOR_GREEN, border_radius=5), ft.Text(caveau.get("reference", "-"), size=15, weight=ft.FontWeight.W_600, color=COLOR_TEXT)], spacing=8),
                            ft.Text(f"Dimensions : {caveau.get('longueur', '-')}m x {caveau.get('largeur', '-')}m", size=12, color=COLOR_TEXT_MUTED),
                        ],
                        spacing=4, expand=True,
                    ),
                    ft.ElevatedButton("Réserver", icon=ft.Icons.ADD_LOCATION_ALT_OUTLINED, bgcolor=COLOR_PRIMARY, color=COLOR_TEXT, on_click=lambda e, c=caveau: open_reservation_dialog(c)),
                ],
    ...
    def refresh_list():
        ...
        if not caveaux or not isinstance(caveaux, list):
            list_container.controls.append(ft.Text("Aucun caveau disponible pour le moment", color=COLOR_TEXT_MUTED, size=14))
        else:
            for c in caveaux:
