...
def exhumations_page(page: ft.Page, on_navigate, on_logout):
    ...
    def build_exhumation_row(ex):
        ...
        if statut == "EN_ATTENTE":
            actions.append(
                ft.IconButton(icon=ft.Icons.CHECK_CIRCLE_OUTLINE, icon_color=COLOR_GREEN, tooltip="Approuver", on_click=lambda e, eid=ex["id"]: open_approuver_dialog(eid))
            )
            actions.append(
                ft.IconButton(icon=ft.Icons.CANCEL_OUTLINED, icon_color=COLOR_RED, tooltip="Refuser", on_click=lambda e, eid=ex["id"]: handle_refuser(eid))
            )
        elif statut == "APPROUVEE":
            actions.append(
                ft.IconButton(icon=ft.Icons.TASK_ALT, icon_color=COLOR_PRIMARY, tooltip="Marquer comme effectuée", on_click=lambda e, eid=ex["id"]: handle_effectuee(eid))
            )
