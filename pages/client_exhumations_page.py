...
def client_exhumations_page(page: ft.Page, on_navigate, on_logout):
    ...
    def open_create_dialog():
        caveau_id_field = ft.TextField(label="ID du caveau", width=320, bgcolor=COLOR_BG, color=COLOR_TEXT, border_color=COLOR_BORDER, keyboard_type=ft.KeyboardType.NUMBER)
        nom_defunt_field = ft.TextField(label="Nom complet du défunt", width=320, bgcolor=COLOR_BG, color=COLOR_TEXT, border_color=COLOR_BORDER)
        motif_field = ft.TextField(label="Motif de la demande", width=320, multiline=True, min_lines=2, bgcolor=COLOR_BG, color=COLOR_TEXT, border_color=COLOR_BORDER)
        date_field = ft.TextField(label="Date d'exhumation souhaitée", width=260, bgcolor=COLOR_BG, color=COLOR_TEXT, border_color=COLOR_BORDER, read_only=True)
        ...
        date_picker = ft.DatePicker(first_date=datetime.now(), last_date=datetime(2100, 1, 1), on_change=handle_date_change)
        page.overlay.append(date_picker)

        date_picker_button = ft.IconButton(icon=ft.Icons.CALENDAR_MONTH_OUTLINED, icon_color=COLOR_PRIMARY, on_click=lambda e: setattr(date_picker, 'open', True) or page.update())
        ...
        def handle_submit(e):
            if not caveau_id_field.value or not nom_defunt_field.value or not motif_field.value:
                ...
                return
            try:
                payload = {"caveau_id": int(caveau_id_field.value), "nom_defunt": nom_defunt_field.value, "motif": motif_field.value}
                if date_field.value:
        ...
        page.dialog = ft.AlertDialog(
            ...
            content=ft.Column([caveau_id_field, nom_defunt_field, motif_field, date_row, error_text, success_text], spacing=12, tight=True, width=340, scroll=ft.ScrollMode.AUTO),
            actions=[
    ...
    def build_row(ex):
        return ft.Container(
            content=ft.Row(
                [
                    ft.Column([ft.Text(ex.get("nom_defunt", "-"), size=14, weight=ft.FontWeight.W_600, color=COLOR_TEXT), ft.Text(f"Caveau ID: {ex.get('caveau_id', '-')} • Motif: {ex.get('motif', '-')}", size=12, color=COLOR_TEXT_MUTED)], spacing=2, expand=True),
    ...
    def refresh_list():
        ...
        if not exhumations or not isinstance(exhumations, list):
            list_container.controls.append(ft.Text("Vous n'avez aucune demande d'exhumation", color=COLOR_TEXT_MUTED, size=14))
        else:
    ...
    header_controls.append(ft.ElevatedButton("Nouvelle demande", icon=ft.Icons.ADD, bgcolor=COLOR_PRIMARY, color=COLOR_TEXT, on_click=lambda e: open_create_dialog()))

    content = ft.Container(
        content=ft.Column([ft.Row(header_controls), ft.Container(height=20), list_container], expand=True),
